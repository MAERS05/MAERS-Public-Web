...
try:
    ...
    HAS_PIL = False
    print("⚠️ [Album] 未检测到 Pillow 库，图片处理功能受限")
...
def sync_gallery_js():
    ...
    try:
        ...
        with open(GALLERY_JS_FILE, 'w', encoding='utf-8') as f:
            f.write(js_content)
        print(f"✅ [Album] Gallery JS 同步成功")
    except Exception as e:
        print(f"❌ [Album] Gallery JS 同步失败: {e}")
...
def handle_upload(query, file_data):
    ...
    if existing_row:
        ...
        if os.path.exists(full_existing_path):
            print(f"♻️ [Album] 检测到重复图片 ({file_hash})，跳过上传，提升排序")
            # 移到第一位 (更新 sort_order)
            # 获取当前最小 order
            ...
            print(f"⚠️ [Album] 数据库存在记录但文件丢失，执行修复: {existing_row['path']}")
...
def handle_delete(body):
    ...
    print(f"🗑️ [Album] 请求删除: {target_path}")
...
def handle_reorder(query, body):
    ...
    try:
        ...
        conn.commit()
        print(f"✅ [Album] 重排序完成 ({len(body)} items)")
        
    except Exception as e:
        print(f"❌ [Album] 重排序失败: {e}")
        conn.rollback()
