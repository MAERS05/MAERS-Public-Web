...
为了提供极致的首屏体验和无闪烁的主题切换，我们采用了特殊的加载策略。
...
*   **加载方式**: 必须在主 CSS 文件的**最顶部**通过 `@import` 引入。
    ...
    .playlist-container { ... }
    ```
*   **权重管理**: 为了确保移动端样式能覆盖主样式（即使 `@import` 在顶部），移动端选择器必须加上 `body` 前缀以提高特异性权重。
    ```css
    /* mobile-music.css */
    body .playlist-container { width: 100%; }
    ```
