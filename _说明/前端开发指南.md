...
- `custom/cms/viewer/cms-common.css` - 通用布局
- `custom/notes/viewer/notes.css` - 笔记特定样式
- `custom/literature/viewer/literature.css` - 文学特定样式

**修改示例：**

```css
/* custom/literature/viewer/literature.css */
body {
  /* 给文学页面加羊皮纸背景 */
...
- `custom/album/viewer/album-viewer.css` - 相册分类卡片样式
- `custom/photos/viewer/photos-viewer.css` - 照片瀑布流与灯箱样式
- `custom/photos/admin/photos-admin.css` - 照片管理模式样式

**关键样式：**

```css
/* custom/photos/viewer/photos-viewer.css */
/* 瀑布流列宽 */
.masonry-column {
...
- `custom/music/viewer/music.css` - 黑胶唱片与播放器样式
- `custom/music/admin/admin-music.css` - 管理模式补充样式
...
│   ├── admin/
│   │   ├── album-admin.js    # 管理逻辑
│   │   └── admin-album.css   # 管理样式
│   └── viewer/
│       ├── album-config.js   # 静态配置
│       └── album-viewer.css  # 视图样式
├── cms/
│   ├── admin/
│   │   ├── cms-editor.js     # 编辑器
│   │   └── cms-drag.js       # 拖拽排序
...
└── index/
    └── admin/
        ├── modules.json      # 模块配置文件
        └── admin-core.js     # 后台核心逻辑
```

### 4.3 HTML 事件绑定

**推荐方式：通过 JS 绑定 (`addEventListener`)**

HTML 文件中应保持纯净，**禁止**出现 `onclick` 等内联属性。
...
```javascript
// custom/index/admin/admin-core.js
document.addEventListener("DOMContentLoaded", () => {
  const btn = document.getElementById("export-btn");
  // 绑定到 MAERS 命名空间的方法
  if (btn) btn.addEventListener("click", MAERS.Admin.exportModules);
});
```
...
**配置文件**: `custom/index/admin/modules.json`
...
```json
{
  ...
  "style": "custom/notes/viewer/notes-cyberpunk.css"
}
```
...
```
1. 创建 custom/[模块]/ 目录 (建立 admin 和 viewer 子目录)
2. 编写 viewer/[模块]-view.js（视图）
3. 编写 admin/[模块]-admin.js（管理）
4. 在 HTML 中引入脚本
5. 注册到 modules.json
```
