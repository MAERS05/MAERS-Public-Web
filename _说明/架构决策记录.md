...
- [ADR-001: 无框架架构 (No Framework Architecture)](#adr-001-无框架架构-no-framework-architecture)
- [ADR-002: SQLite 数据库 (SQLite Database)](#adr-002-sqlite-数据库-sqlite-database)
- [ADR-003: 前后端混合架构 (Hybrid Architecture)](#adr-003-前后端混合架构-hybrid-architecture)
- [ADR-004: 模块化目录结构 (Modular Structure)](#adr-004-模块化目录结构-modular-structure)
- [ADR-005: ES6 模块化 (ES6 Modules)](#adr-005-es6-模块化-es6-modules)
- [ADR-006: 关键路径同步渲染 (Flash Guard)](#adr-006-关键路径同步渲染-flash-guard)
- [ADR-007: CSS 分层架构 (CSS Layering)](#adr-007-css-分层架构-css-layering)
- [ADR-008: 图片多级存储 (Multi-level Image Storage)](#adr-008-图片多级存储-multi-level-image-storage)
- [ADR-009: 统一数据交换格式为 JSON (Unified Data Format)](#adr-009-统一数据交换格式为-json-unified-data-format)
...
- [ADR-013: 事件驱动 UI 架构 (Event-Driven UI)](#adr-013-事件驱动-ui-架构-event-driven-ui)
...
- [ADR-017: 移动端极致性能降级 (Mobile Extreme Performance Degradation)](#adr-017-移动端极致性能降级-mobile-extreme-performance-degradation)
- [ADR-018: 统一 SVG 图标体系 (Unified SVG Icon System)](#adr-018-统一-svg-图标体系-unified-svg-icon-system)
- [ADR-019: 核心主题与脚本统一 (Theme Unification)](#adr-019-核心主题与脚本统一-theme-unification)
- [ADR-020: 交互式身份验证物理删除 (Secure Deletion Confirmation)](#adr-020-交互式身份验证物理删除-secure-deletion-confirmation)
- [ADR-021: Studio Engine 标准化双语日志回显 (Standardized Logging)](#adr-021-studio-engine-标准化双语日志回显-standardized-logging)
- [ADR-022: 模块微服务化重构与职责隔离 (Micro-Modular Refactoring)](#adr-022-模块微服务化重构与职责隔离-micro-modular-refactoring)
- [ADR-023: 管理端物理操作反馈增强 (Associated Feedback)](#adr-023-管理端物理操作反馈增强-associated-feedback)
...
- [ADR-029: JSON 驱动标签配置 (JSON-based Tag Configuration Storage)](#adr-029-json-驱动标签配置-json-based-tag-configuration-storage)
- [ADR-030: 标准浏览器级懒加载 (Standard Browser-level Lazy Loading)](#adr-030-标准浏览器级懒加载-standard-browser-level-lazy-loading)
- [ADR-031: 原子化通知系统 (Atomic Notification System)](#adr-031-原子化通知系统-atomic-notification-system)
- [ADR-032: 标签配置命名规范 (Tag Configuration Naming Convention)](#adr-032-标签配置命名规范-tag-configuration-naming-convention)
- [ADR-033: Videos 影视模块规范与交互增强 (Videos Module Standards)](#adr-033-videos-影视模块规范与交互增强-videos-module-standards)
...
## ADR-033: Videos 影视模块规范与交互增强 (Videos Module Standards)

**状态**: ✅ 已采纳 / 2026-03-07 新增
**类型**: 视觉规范 / 交互逻辑

### 背景
随着影视内容（电影、电视剧、纪录片）的引入，传统的 1:1 或 4:3 网格布局无法满足海报展示的审美需求。同时，影视内容与音乐内容（Music）在娱乐维度上具有高度相关性，需要更便捷的导航跳转。

### 决策
1. **海报级纵横比 (2:3 Aspect Ratio)**: 规定 `Videos` 模块（及文学、游戏模块）统一采用 **2:3** 竖屏海报比例。
   - **理由**: 适配行业标准的电影海报规格，提升内容的专业感。
2. **交互式页面标题 (Interactive Page Title)**: 
   - 增强 `cms-render-nav.module.js` 的渲染能力，允许在 `titleMap` 中配置安全的 HTML 片段。
   - 实现 "Music & Videos" 联动标题，用户点击标题特定区域可实现模块间快速切换。
3. **元数据抓取对齐 (API Pluralization)**:
   - 后端 API 内部继续使用 `get_video_info` 保持向后兼容。
   - 上传路径统一为 `videocovers` 以保持目录结构的简洁性。

### 理由
1. **视觉一致性**: 确立了除普通文档外的“海报类内容”视觉基准。
2. **导航效率**: 打破了 CMS 模块间的导航孤岛，实现了娱乐模块的逻辑聚合。
3. **安全性平衡**: 在维持 XSS 防护的前提下，通过对受控配置项渲染 HTML，平衡了灵活性与安全性。
