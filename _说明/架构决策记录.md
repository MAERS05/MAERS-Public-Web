...
### 决策
1.  **合并与废弃**: 将 `script.module.js` 的所有核心功能（Zoom, Drag, Iframe Click Handling）迁移至 `custom/shared/ui/theme.module.js`，并彻底删除旧脚本。
...
*   **Server-First 模式 (CMS / Photos 模块)**
    ...
    *   **机制**: **SQLite 数据库托管**。
        *   CMS: `data/cms.db` (存储文章节点树)
        *   Photos: `data/gallery.db` (存储图片元数据与哈希)
    *   **特性**: 
        *   **立即生效 (Immediate Write)**: 操作通过 SQL 立即执行并持久化。
        *   **依赖后端**: 必须在 Python 服务 (SQL) 运行时才能写入。
        *   **数据源**: 数据库是单一事实来源。`*-tree.json` 和 `photos-data.json` 均由后端自动生成。
