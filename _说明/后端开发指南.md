...
- [7. 管理操作手册](#7-管理操作手册-management-operations-manual)
  - [7.7 Markdown 内部链接与跨模块跳转](#77-markdown-内部链接与跨模块跳转)
...
├── services/           # [核心] 业务逻辑封装
│   ├── cms.py            # [入口] CMS服务分发层
│   ├── cms_nodes.py      # [核心] 节点与物理文件同步逻辑
│   ├── cms_tags.py       # [核心] 标签库持久化与查询
│   ├── cms_other_tags.py # [工具] 标签重命名、删除与清理逻辑
│   ├── album.py          # 相册分类管理服务
│   ├── photos.py         # 图片处理与上传服务 - SQLite 驱动
│   ├── space.py          # 空间模块服务
│   ├── music.py          # 音乐管理服务
│   └── music_api.py      # 音乐外部 API 接口 (Bilibili 等)
...
### 3.1 CMS Service (`cms_*.py`)
负责 Notes, Literature, Record, Games, Videos 五大内容模块，采用模块化分工：
- **CMS Logic (`cms_nodes.py`)**: 所有的节点层级、标题均存储在 `cms.db`。正文内容存储为指向磁盘 `.md` 文件的相对路径。负责 Add/Update/Rename/Delete 物理同步。
- **Tag System (`cms_tags.py`)**: 持久化管理，不再存储于数据库，而是存储在 `data/tags/` JSON 文件中。
- **Tag Ops (`cms_other_tags.py`)**: 专门处理重命名、删除及跨数据源的 `cleanup_unused_tags` 逻辑。
- **Static Snapshot**: 每次写入操作后，重新生成 JSON 树状文件（如 `data/notes-tree.json`）。
...
| :--- | :--- | :--- | :--- |
| `GET` | `/api/cms/fetch` | `cms.fetch_module_tree` | 获取指定模块 (Notes/Lit/Record/Videos) 的文件树。 |
...
| `GET` | `/api/get_bili_info` | `music_api.get_video_info` | 获取外部视频/音乐信息请求代理。 |
...
*   `[ MUSIC ]`: 涉及音乐数据同步、音轨管理。
*   `[ BILI ]`: 涉及外部 API (Bilibili, Music 等) 请求过程。
...
### 7.2 CMS 内容管理 (笔记/文学/游戏/视频/记录)
适用于 `notes`, `literature`, `games`, `videos`, `record` 五大模块。
...
*   **标签管理**:
    ...
    2.  **右键菜单 (单个标签操作)**:
        - **重命名 (✎)**: 右键点击标签 -> 输入新名称。系统会自动更新所有使用该标签的项目（笔记/照片等）。
        - **排序 (⇅)**: 右键点击标签 -> 点击该分类下的另一个标签。选中的标签将插在目标标签后面。
        - **删除 (✕)**: 右键点击标签 -> 确认。系统会从所有项目中移除该标签。
    3.  **新建/重命名分类**: 只有在管理员模式下，抽屉底部会出现 `+ 添加分类` 按钮。点击分类标题旁的 `✎` 可重命名。
    4.  **移动标签 (批量操作)**: 选中多个标签（点击左侧圆点）后，点击分类标题右侧的 `移动至此` 按钮。
    5.  **清理未使用标签**: 点击抽屉顶部的垃圾桶图标 (🗑️)，扫描并移除未被引用的标签。
    6.  *注意*: 所有变更会自动持久化至 `data/tags/*.json`。
*   **新增模块** (如需新增 `blog`):
...
### 7.7 Markdown 内部链接与跨模块跳转

MAERS 支持在 Markdown 文章中嵌入两种跳转链接，帮助读者在模块之间无缝漫游。

#### ① 同模块内跳转（最简单）

**适用场景**：在 Notes 里链接另一篇 Notes，在 Games 里链接另一款游戏。

直接使用相对路径写文件名，系统会自动拦截并在当前页面打开：
...
> ⚠️ **注意**：文件名必须与笔记的**标题完全一致**（含标点、书名号等）。

---

#### ② 跨模块跳转（`?openNote` 参数）

**适用场景**：在 Notes 里引用 Literature 的一本书、在 Games 里链接到 Videos 里的同名影视化作品。

使用**相对路径 + `?openNote=` 参数**的格式书写：

```markdown
[黑神话：悟空](/games.html?openNote=黑神话：悟空)
[《北欧神话》](/literature.html?openNote=《北欧神话》)
[雷神系列](/videos.html?openNote=《雷神系列》)
```

点击后，浏览器会跳转到对应模块页面，页面加载完成后自动弹开匹配标题的详情窗口。

> ⚠️ **注意**：
> - `openNote` 的值必须与节点的 `title` 字段**完全一致**（含书名号等特殊字符）。
> - 推荐使用**相对路径**（以 `/` 开头），这样在本地测试（`localhost:8000`）和正式域名（`maersy.com`）下都能正常工作，无需修改链接。
...
| 页面 | 使用链接格式 |
| :--- | :--- |
| 笔记 (Notes) | `/notes.html?openNote=标题` |
| 文学 (Literature) | `/literature.html?openNote=标题` |
| 游戏 (Games) | `/games.html?openNote=标题` |
| 影视 (Videos) | `/videos.html?openNote=标题` |
| 后台编辑（管理员专用）| `/admin-cms.html?module=games&openNote=标题` |
