...
| `data/` | `index-cards.json` | **首页卡片数据**。定义首页显示的各种导航卡片及其链接。 |
...
| `custom/admin-portal/viewer/` | `admin.css` | **管理样式**。包含管理后台的布局、卡片样式及主题适配逻辑。 |
...
| `spatial-nav.css` | **空间导航样式**。定义侧边 3D 悬浮导航的布局与动画，包含**主题图标适配逻辑**。 |

### 9.3 全局共享逻辑 (shared/ & data-manage/)
| 文件名 | 路径 | 描述 |
| :--- | :--- | :--- |
| `admin-portal.module.js` | `custom/admin-portal/admin/` | **[CORE] 首页模块管理**。负责全站模块的增删改查、图标配置、页面自动创建及物理删除确认。 |
