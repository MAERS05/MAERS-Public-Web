...
| `videos.html` | Entry | **视频 (CMS)**。采用 2:3 海报流布局展示影视作品。 |
...
| `install_deps.py` | **依赖安装脚本**。自动检查并安装项目所需的第三方 Python 库（如 Pillow 等）。 |
| `安装所需库(install_deps).bat` | **环境配置脚本**。Windows 下快速执行依赖安装的批处理文件。 |
| `config.py` | **配置文件**。定义端口号、数据库路径、上传目录等全局常量。 |
| `open-file.py` | **文件定位**。接收文件路径参数，调用系统默认程序打开文件或所在目录。 |
| `services/cms.py` | **CMS服务入口**。统一分发至各子服务。管理节点层级与物理文件同步的通用接口。 |
| `services/cms_nodes.py`| **节点服务**。处理 Notes/Lit/Record 等模块的 MD 文件同步与层级逻辑。 |
| `services/cms_tags.py` | **标签库服务**。核心标签分类逻辑、JSON 持久化与全局标签库查询。 |
| `services/cms_other_tags.py`| **标签工具**。处理标签清理、重命名、删除等批量/工具类任务。 |
...
| `services/music_api.py` | **音乐 API 服务**。集成各类平台（如 Bilibili）的元数据抓取与解析。 |
...
| `admin-main.module.js` | `root` | **管理入口**。初始化 CMS 编辑器与管理功能。按当前模块 (`CURRENT_MODULE`) 自动分发对应的 `CustomView`（支持 `literature`、`games` 等）。 |
...
| `render/cms-render-grid.module.js`| **网格渲染**。文件夹与文件卡片生成。`createGridItem` 已对外导出，可被 Games 等模块复用。 |
...
| `tags/cms-tags-drawer.module.js`| **标签抽屉主控**。协调渲染、UI与数据同步的核心 Controller。 |
| `tags/cms-tags-render.module.js`| **标签渲染层**。负责抽屉 HTML 生成、计数统计与分类展示。 |
| `tags/cms-tags-ui.module.js`| **标签交互层**。处理右键菜单、拖拽排序模式与视觉反馈。 |
| `tags/cms-tags-filter.module.js`| **标签筛选逻辑**。筛选算法封装。 |
...
| `music-admin.module.js` | `admin/` | **管理**。调用接口获取数据（如 B站）并管理本地歌单。 |
...
| `games-view.module.js` | `viewer/` | **视图引擎**。管理 Playing / Played 双视图切换。Playing 视图含随机抽取逻辑，并将结果通过 `localStorage` 持久化，下次访问自动恢复。 |
| `games.css` | `viewer/` | **游戏样式**。定义网格布局、卡片悬停效果、Playing 沙盒布局与抽取按钮样式。 |
| `games-reset.css` | `viewer/` | **重置样式**。定制化 CMS 基础样式以适配游戏封面展示需求。 |

### 3.8 视频模块 (videos/)
| 文件名 | 路径 | 描述 |
| :--- | :--- | :--- |
| `main.module.js` | `root` | **主入口**。初始化 Videos 视图，集成与 Music 模块的联动逻辑。 |
| `videos.css` | `viewer/` | **视频样式**。定义 **2:3 竖屏海报网格**、卡片悬停反馈及 Admin 按钮布局。 |
...
| `mobile-videos.css` | **视频**。优化海报流在窄屏下的展示效率。 |
...
| `data/videos-tree.json`| **Videos 结构**。视频模块的树状索引（不含正文）。 |
| `data/videos/` | **视频正文**。存储影视模块的 `.md` 源文件。 |
