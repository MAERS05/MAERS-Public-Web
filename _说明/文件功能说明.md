...
| `custom/index/admin/` | `index-admin.module.js`| **首页管理模块**。负责首页卡片 (`index-cards.json`) 的增删改查。 |
| `data-manage/` | `admin-core.module.js` | **首页核心逻辑**。处理主页模块的数据读写、页面物理创建与销毁。 |
| `data-manage/` | `admin-base.module.js` | **Admin Base**。全站 Admin 的公共工具类，提供 BatchManager, SaveButton 等通用功能。 |
| `custom/index/admin/` | `admin.css` | **管理样式**。包含首页卡片编辑态的图标动画、操作按钮及遮罩样式。 |
...
| `mobile-index.css` | `index.css` | **首页适配**。处理 Logo/Slogan 在缩放视图 (`shrink-view`) 下的防裁剪与布局调整。 |
...
| 文件名 | 描述 |
| :--- | :--- |
| `flash-guard.js` | **[CRITICAL] 防闪烁守卫**。HTML 头部同步阻塞脚本，在渲染前注入遮罩和主题类，防止 FOUC。整合了 LocalStorage 读取、Favicon 注入和主题初始化。 |
| `style-injector.module.js`| **样式注入器**。负责动态加载模块特定的 CSS 文件。 |
| `module-config.module.js` | **配置中心**。定义了 CMS 的所有元数据（图标、允许的特性、数据文件路径）。 |
...
### 9.3 全局共享逻辑 (shared/ & data-manage/)
| 文件名 | 路径 | 描述 |
| :--- | :--- | :--- |
| `admin-core.module.js` | `data-manage/` | **[CORE] 首页模块管理**。负责全站模块的增删改查、图标配置、页面自动创建及物理删除确认。 |
| `admin-base.module.js` | `data-manage/` | **[BASE] 管理后台基石**。提供 `BatchItemManager`, `SaveButton`, `AdminButtonHelper` 等全站管理端通用的列表操作工具。 |
| `simple-main.module.js` | `shared/` | **通用页面入口**。为 Me, Space, Games 等静态页面提供初始化和 Header 渲染。 |
| `layout.module.js` | `shared/` | **布局初始化**。渲染公共 Header/Menu。 |
| `theme.module.js` | `shared/` | **主题管理中心**。整合了深色模式、全局缩放 (Drag & Pan)、Iframe 点击穿透逻辑。是 Admin 页面的核心依赖。 |
| `spatial-nav.module.js` | `shared/` | **空间导航**。为非首页页面提供 3D 悬浮导航球。 |
| `utils.module.js` | `shared/` | **公共工具库**。包含 HTML 转义 (XSS 防护)、日期格式化、拼音查词匹配等核心工具。 |
| `toast.module.js` | `shared/` | **全局通知服务**。提供 `MAERS.Toast` 单例，实现在任意位置弹出消息反馈。 |

### 9.4 UI 资源 (ui/)
| 文件名 | 描述 |
| :--- | :--- |
| `*.svg` | **图标资源**。包括 `music-home.svg`, `cms-history.svg`, `cms-pin.svg` 等，使用外部 SVG 引用以替代硬编码字符串。 |

### 9.5 数据与管理 (data-manage/)
| 文件名 | 描述 |
| :--- | :--- |
| `api-client.module.js` | **API 客户端**。统一封装 `fetch` 请求，处理 GET/POST，可视为简化的 Axios。 |
