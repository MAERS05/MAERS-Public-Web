...
| `custom/index/admin/` | `index-admin.module.js`| **首页管理模块**。负责首页卡片 (`index-cards.json`) 的增删改查及模块配置。 |
| `custom/index/admin/` | `admin-core.module.js` | **Admin Core**。全站 Admin 的公共基类，提供 BatchManager, SaveButton 等通用功能。 |
...
### 9.3 全局共享逻辑 (custom/shared/)
| 文件名 | 描述 |
| :--- | :--- |
| `admin-core.module.js` | **[CORE] 管理后台基石**。提供批量选择、全局保存条、操作按钮生成器和统一反馈接口。已实现 SVG 路径自动包装和多步弹窗原子化撤销逻辑。|
| `module-config.module.js`| **配置中心**。定义了 CMS 的所有元数据（图标、允许的特性、数据文件路径）。|
| `ui/layout.module.js` | **布局初始化**。渲染公共 Header/Menu。 |
| `ui/theme.module.js` | **主题管理中心**。整合了深色模式、全局缩放 (Drag & Pan)、Iframe 点击穿透逻辑及 `drag.css` 的自动注入。是 Admin 页面的核心依赖。 |
