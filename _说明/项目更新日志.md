...
## [v5.0] - 2026-02-12
### 🚀 性能与体验突破 (Performance & UX Breakthrough)
- **CMS 标签系统 3.0**: 实现了高度灵活的模块化标签存储。
    - **逻辑解耦**: 标签配置独立存储于 `data/tags/` 下的 JSON 文件，与内容树分离。
    - **稳定性**: 增强了 `cms-controller` 的容错逻辑，支持静态/离线环境下的标签路径自动补全。
- **图像渲染架构重构 (Image Rendering)**:
    - **Native Lazy Loading**: 全站废弃 JS 模拟的懒加载方案。全面拥抱浏览器原生的 `loading="lazy"` 属性，大幅降低 JS 执行时间。
    - **语义化标签**: 废弃 `div.background-image` 方案，改用 `<img>` 标签配合 `object-fit: cover`。这不仅解决了右键无法“保存图片”的痛点，还显著提升了 SEO 和无障碍访问性。
- **原子化通知系统 (Atomic Notifications)**:
    - **反馈解耦**: 将原本聚合的“保存成功”通知拆解为原子化的独立消息。
    - **物理感知**: 当删除操作触发服务器文件清理时，会弹出独立的、高亮的 Toast 告知用户“关联物理文件已粉碎”，增强系统的透明度。
