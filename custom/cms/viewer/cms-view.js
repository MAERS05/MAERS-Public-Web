  ...
  const CmsView = (function () {
    ...
    function uiPickNode(e, id) {
      ...
      if (AppState.pickedId === id) uiCancelPick();
        ...
        renderGrid(
          AppState.pathStack.length > 1
            ? AppState.pathStack[AppState.pathStack.length - 1].children
            : AppState.root,
        );
        startGhost(id);
      }
    ...
    async function uiExecuteMove(targetParentId) {
      if (targetParentId !== AppState.pickedId) {
        const res = await MAERS.CMS.Controller.moveNode(
          AppState.pickedId,
          targetParentId,
        );
        if (res.success) {
    ...
    async function refreshView() {
      ...
      if (currentId !== "root") {
        const newPath = MAERS.Utils.Search.findPathNodes(
          AppState.root,
          currentId,
        );
    ...
    function startGhost(id) {
      ...
      if (!g) {
        ...
        g.id = "move-ghost";
        g.style.cssText =
          "position:fixed;z-index:9999;pointer-events:none;background:rgba(120,255,214,0.9);color:#000;padding:8px 12px;border-radius:20px;box-shadow:0 4px 15px rgba(0,0,0,0.3);font-weight:bold;font-size:14px;backdrop-filter:blur(5px);border:1px solid rgba(255,255,255,0.5);display:none;";
    ...
    function refreshDrawerList() {
        ...
        (node.tags || []).forEach((t) => {
          ...
          if (query) {
            const keys = query
              ...
              .filter((s) => s);
            if (keys.length > 0)
              match = keys.some((k) => t.toLowerCase().includes(k));
          }
      ...
      if (sorted.length === 0) {
        listContainer.innerHTML =
          '<div style="text-align:center;color:#666;margin-top:20px;">No tags found</div>';
      } else {
    ...
    function setupGridEventDelegation() {
      ...
      container.addEventListener("click", (e) => {
        ...
        if (card) {
          ...
          else if (MAERS.CMS.Core && MAERS.CMS.Core.openNote)
            MAERS.CMS.Core.openNote(node);
          else if (MAERS.CMS.Editor && MAERS.CMS.Editor.openNote)
            MAERS.CMS.Editor.openNote(node);
          else if (MAERS.CMS.Editor && MAERS.CMS.Editor.open)
            MAERS.CMS.Editor.open(node);
          return;
        }
      });
    ...
    function renderGrid(list, isSearch = false) {
      ...
      function renderBatch() {
        ...
        for (let i = index; i < end; i++) {
          ...
          el.innerHTML = `${adminActions}<div class="item-title">${safeTitle}</div><div class="item-tags">${tagsHtml}</div>`;
    ...
    function renderPageTitle() {
      ...
      if (MAERS.ModuleConfig) {
        titleEl.textContent = MAERS.ModuleConfig.getTitle(
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (tagToggleBtn)
      tagToggleBtn.addEventListener("click", () =>
        MAERS.CMS.View.toggleTagDrawer(),
      );
    if (overlay)
      overlay.addEventListener("click", () => MAERS.CMS.View.toggleTagDrawer());
    if (closeDrawerBtn)
    ...
    editorCloseBtns.forEach((btn) => {
      btn.addEventListener(
        "click",
        () => MAERS.CMS.Editor && MAERS.CMS.Editor.close(),
      );
    });
  });
