/**
 * MAERS Music Control (music-control.module.js)
 * 播放控制逻辑 (Next/Prev)
...
export function initControl(playerModule, uiModule) {
    Player = playerModule;
    UI = uiModule;
}
...
export function playNext() {
    ...
    if (catIdx + 1 < UI.libraryData.length) {
        ...
        if (nextCat.collections.length > 0) {
            ...
            if (nextCol.albums.length > 0) {
                ...
                if (UI.enterLevel2) setTimeout(() => UI.enterLevel2(nextCatIdx, 0), ANIMATION_DELAY);
                const totalP = parseInt(nextAlbum.total) || 1;
                const nextMapping = nextAlbum.page_mapping || Array.from({ length: totalP }, (_, k) => k + 1);
    ...
    if (UI.libraryData.length > 0) {
        ...
        if (UI.enterLevel2) setTimeout(() => UI.enterLevel2(0, 0), ANIMATION_DELAY);
    ...
    if (catIdx > 0) {
        ...
        if (prevCat.collections.length > 0) {
            ...
            if (lastCol.albums.length > 0) {
                ...
                if (UI.enterLevel2) setTimeout(() => UI.enterLevel2(prevCatIdx, lastColIdx), ANIMATION_DELAY);
        ...
        if (cat.collections.length > 0) {
            for (let j = cat.collections.length - 1; j >= 0; j--) {
                ...
                if (col.albums.length > 0) {
                    ...
                    if (UI.enterLevel2) setTimeout(() => UI.enterLevel2(i, j), ANIMATION_DELAY);
...
document.addEventListener('DOMContentLoaded', () => {
    Utils.bindEvent('#play-prev-btn', 'click', playPrev);
    Utils.bindEvent('#play-next-btn', 'click', playNext);
});
